...
inline void glass2Init() {
    ...
    if (_g2_ready) {
        ...
        _g2.setFont(&fontRUsmall);  // 6x12 с кириллицей
...
inline void glass2Show(const char* l1, const char* l2 = nullptr) {
    if (!_g2_ready) return;
    _g2.fillScreen(TFT_BLACK); _g2.setFont(&fontRUsmall); _g2.setTextColor(TFT_WHITE, TFT_BLACK);
...
inline void glass2Spectrum(const char* name, const uint8_t* bars, uint8_t numBars) {
    ...
    _g2.setFont(&fontRUsmall); _g2.setTextColor(TFT_WHITE, TFT_BLACK);
    if (name) { _g2.setCursor(0, 1); _g2.print(name); }
